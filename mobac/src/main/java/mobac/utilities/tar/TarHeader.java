...
public class TarHeader {
	...
	// private static Logger log = LoggerFactory.getLogger(TarHeader.class);
	...
	public void setFileName(String newFileName) {
		...
		if (newFileNameLength > fileName.length) {
			throw new RuntimeException(
					"File name length exceeds limit of 100 characters: " + newFileNameLength + "chars:" + newFileName);
		}
	 ...
	 * by a null and a space character. Note that many early implementations of tar
	 * used signed arithmetic for the checksum field, which can cause inter-
	 * operability problems when transferring archives between systems. Modern
	 * robust readers compute the checksum both ways and accept the header if either
	 * computation matches.<br>
	 * <a href=
	 ...
	 *
	 * @param header
	 *            array containing a tar header at offset 0 (512 bytes of size) with
	 *            prepared checksum field (filled with spaces)
	 */
	...
	public byte[] getBytes() {
		...
		byte[] result = sb.toString().getBytes(StandardCharsets.US_ASCII);

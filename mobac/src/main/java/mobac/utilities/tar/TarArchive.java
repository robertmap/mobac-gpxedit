...
/**
 * Creates a new tar file and allows to add files from the file system or "virtual files" that only exist in memory as
 * <code>byte[]</code>.
 */
public class TarArchive implements Closeable {

    /**
     * 128 KB default buffer
     */
    private static final int BUFFER_SIZE = 1024 * 128;

    protected final CountingOutputStream tarFileStream;
    protected final File tarFile;
    protected final File baseDir;

    /**
     * @param tarFile
     * @param baseDir root directory used for getting the relative path when adding a file from the file system. If only in
     *                memory files are added this parameter can be <code>null</code>
     * @throws FileNotFoundException
     */
    public TarArchive(File tarFile, File baseDir) throws FileNotFoundException {
        this.tarFile = tarFile;
        this.tarFileStream = new CountingOutputStream(
                new BufferedOutputStream(new FileOutputStream(tarFile, false), BUFFER_SIZE));

...
public class OSUtilities {
    ...
    public static String getLinuxDistributionName() {
        try {
            ...
            List<Path> releaseFileList = Files.walk(etcDir).filter(f -> pattern.matcher(f.getFileName().toString()).matches()).collect(Collectors.toList());
            if (releaseFileList.isEmpty()) {
                return null;
            }
            Path releaseFile;
            if (releaseFileList.size() > 1) {
                Optional<Path> optional = releaseFileList.stream().filter(f -> f.getFileName().toString().contains("lsb")).findFirst();

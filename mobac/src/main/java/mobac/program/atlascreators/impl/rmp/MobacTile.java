...
public class MobacTile {
	private static final Logger log = LoggerFactory.getLogger(MobacTile.class);
	...
	public void drawSubImage(BoundingRect dest_area, BufferedImage dest_image) {
		...
		/*
		 * --- Iterate over all pixels of the destination image. Unfortunately we need
		 * this technique because source and dest do not have exactly the same zoom
		 * level, so the source image has to be compressed or expanded to match the
		 * destination image ---
		 */

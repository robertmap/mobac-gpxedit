...
public class PNGWorldfile extends Glopus {
	...
	private void writeWorldFile() throws MapCreationException {
		try (Writer mapWriter = new OutputStreamWriter(new FileOutputStream(new File(layerDir, mapName + ".pgw")),
				TEXT_FILE_CHARSET)) {
	...
	private void writeProjectionFile() throws MapCreationException {
		File xmlFile = new File(layerDir, mapName + ".png.aux.xml");
		try (OutputStreamWriter writer = new OutputStreamWriter(new FileOutputStream(xmlFile), TEXT_FILE_CHARSET)) {

 ...
 * <p>
 * BEGIN TRANSACTION; CREATE TABLE images(zoom int, x int, y int, flags int,
 * length int, data blob); CREATE TABLE version(version int); INSERT INTO
 * version VALUES(5); INSERT INTO version VALUES(0);
 * <p>
 * CREATE INDEX index1 on images (zoom,x,y,flags); COMMIT;
 * <p>
 * The columns in "images" table have the following meaning:
 * <p>
 * zoom - zoom level from 1 (top level) to 23 (most detailed level). On the top
 * level we have four squares 128x128 pixels, it gives 512x512 pixels. So map
 * width = map height = 256 * 2zoom pixels.
 * <p>
 * x, y - coordinates for given zoom level:
 * <p>
 * <p>
 * <p>
 * flags - layer number. Theoretically map could contain number of layers:
 * street, satellite, hybrid and so on. Practically the program shows only one
 * layer with smallest flags value.
 * <p>
 * length - size of binary image in bytes.
 * <p>
 * data - binary tile image in PNG or JPEG format. Practically the program
 * supports only PNG for now.
 * <p>
 * For more details regarding Web Mercator projection and coordinates please
 * see:
 * <p>

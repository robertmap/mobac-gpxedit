 ...
 * <li>1 to n times {@link #initializeMap(MapInterface, TileProvider)} followed
 * by {@link #createMap()}</li>
...
public abstract class AtlasCreator {
	 ...
	 *            if not <code>null</code> the customAtlasDir is used instead of the
	 *            generated atlas directory name
	...
	public void startAtlasCreation(AtlasInterface atlas, File customAtlasDir)
		...
		if (customAtlasDir == null) {
			// No explicit atlas output directory has been set - generate a new directory
			// name
	...
	/**
	 * Tests all maps of the currently active atlas if a custom tile image format
	 * has been specified and if the specified format is equal to the

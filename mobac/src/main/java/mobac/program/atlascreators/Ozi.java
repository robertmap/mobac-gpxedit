...
public class Ozi extends AtlasCreator {
	...
	protected void writeMapFile() {
		File mapFile = new File(layerDir, mapName + ".map");
		try (FileOutputStream fout = new FileOutputStream(mapFile)) {
	...
	protected void writeMapFile(String imageFileName, OutputStream stream) throws IOException {
		...
		// Supplied by MrPete, based on suggestion from Des Newman, author of
		// OziExplorer
		// Set latitude to midpoint (maxLat+minLat/2)
		double midLat = Math.toRadians((latitudeMax + latitudeMin) / 2.0);

		// Calculate 50 pixel Longitude for interpolation: Lon50 = 50/(ImagePixelWidth)
		// * abs(maxLon - minLon)
		...
		// Set lonW and lonE to midpoint +/1 50 pixels: lonW = midLon - Lon50; lonE =
		// midLon+Lon50
		...
		// For the final scaling, convert to distance in meters (multiply by earth
		// radius),
	...
	/**
	 * Writes the large picture (tile) line by line. Each line has the full width of
	 * the map and the height of one tile (256 pixels).
	 */
	protected void createTiles() throws InterruptedException, MapCreationException {
		...
		try (FileOutputStream fileOs = new FileOutputStream(new File(layerDir, mapName + ".png"))) {

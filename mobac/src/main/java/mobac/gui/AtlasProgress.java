...
public class AtlasProgress extends JFrame implements ActionListener, MapSourceListener {
	...
	private void createComponents() {
		...
		openProgramFolderButton
				.setToolTipText(I18nUtils.localizedStringForKey("dlg_download_btn_open_folder_tips_disabled"));
		...
		bottomPanel.add(infoPanel, GBC.std().gridheight(2).fill());
	...
	private class GUIUpdater implements Runnable {
		...
		public void run() {
			...
			if (data.atlasInterface != null) {
				windowTitle.setText(
						I18nUtils.localizedStringForKey("dlg_download_window_title", data.atlasInterface.getName()));
			}
			...
			atlasTimeLeft.setText(I18nUtils.localizedStringForKey("dlg_download_remain_time", formatTime(seconds)));
			...
			mapDownloadPercent
					.setText(String.format(TEXT_PERCENT, (int) (mapDownloadProgressBar.getPercentComplete() * 100)));

			mapDownloadElementsDone
					.setText(String.format(I18nUtils.localizedStringForKey("dlg_download_tile_done_count"),
			...
			if (data.mapCreationProgress == 0 && initialMapDownloadTime > 0) {
				if (time == 0) {
					...
					nrOfDownloadedBytesPerSecondValue
							.setText(String.format(I18nUtils.localizedStringForKey("dlg_download_avg_speed_value"),
			...
			retryableDownloadErrorsValue
					.setText(String.format(I18nUtils.localizedStringForKey("dlg_download_retry_count_value"),
			...
			permanentDownloadErrorsValue
					.setText(String.format(I18nUtils.localizedStringForKey("dlg_download_failed_count_value"),

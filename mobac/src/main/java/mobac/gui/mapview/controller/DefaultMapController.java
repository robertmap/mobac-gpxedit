...
public class DefaultMapController extends JMapController
    ...
    public void mousePressed(MouseEvent e) {
        if (e.getButton() == movementMouseButton || OSUtilities.IS_PLATFORM_OSX && e.getModifiersEx() == MAC_MOUSE_BUTTON3_MASK) {
            lastDragPoint = null;
            isMoving = true;
        }

        // add ruler point
        if (map.isMeasuring && e.getButton() == 1) {
            map.ruler.insert(lastPoint);
            map.repaint();
        }
    }

    public void mouseReleased(MouseEvent e) {
        if (e.getButton() == movementMouseButton || OSUtilities.IS_PLATFORM_OSX && e.getButton() == MouseEvent.BUTTON1) {
            lastDragPoint = null;
            isMoving = false;
        }
    }
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        if (!OSUtilities.IS_PLATFORM_OSX || !movementEnabled || !isMoving) {
            return;
        }

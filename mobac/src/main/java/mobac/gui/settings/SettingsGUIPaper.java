...
public class SettingsGUIPaper extends JPanel {
	...
	private static final Logger LOG = LoggerFactory.getLogger(SettingsGUIPaper.class);
	...
	public SettingsGUIPaper() {
		...
		LOG.debug("Preparing paper atlas settings panel");
	...
	private void exportToXml() {
		...
		if (state == JFileChooser.APPROVE_OPTION) {
			File file = jFileChooser.getSelectedFile();
			try {
				JAXBContext context = JAXBContext.newInstance(SettingsPaperAtlas.class);
	...
	private void i18n() {
		...
		jCheckBoxWgsGrid.setText(I18nUtils.localizedStringForKey("set_paper_wgs_grid"));

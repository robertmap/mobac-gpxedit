 ...
 * pixel. This is the common projection used by OpenStreetMap and Google. It
...
public class MercatorPower2MapSpace implements MapSpace {
	...
	/**
	 * Returns the absolute number of pixels in y or x, defined as: 2<sup>zoom</sup>
	 * * <code>tileSize</code>
	 *
	 * @param zoom
	...
	public double distance(double lat1, double lon1, double lat2, double lon2) {
		...
		UnitSystem unit = Settings.getInstance().unitSystem;

		double theta = lon1 - lon2;
		double lat1Rad = Math.toRadians(lat1);
		double lat2Rad = Math.toRadians(lat2);
		double dist = Math.sin(lat1Rad) * Math.sin(lat2Rad)
				+ Math.cos(lat1Rad) * Math.cos(lat2Rad) * Math.cos(Math.toRadians(theta));
		dist = Math.toDegrees(Math.acos(dist)) * 60 * 1.1515;
		dist *= unit == UnitSystem.Metric ? 1.609344d : 0.8684d;
